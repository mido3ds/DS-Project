		...
		while (true)
		{
			...
			switch (R)
			{
				case 'A':
				{
					...
					if (ea == NULL)
					{
						ea = ENEMY::Initialize(S, TY, T, H, Pow, Prd, static_cast<int>(A_REG));
						TA.firstEnemy = ea;
						TA.num_enemies++;
						continue;
					}
					...
					temp = ENEMY::Initialize(S, TY, T, H, Pow, Prd, static_cast<int>(A_REG));
					ea->next = temp;
					TA.num_enemies++;
					break;
				}
				case 'B':
				{
					...
					if (eb == NULL)
					{
						eb = ENEMY::Initialize(S, TY, T, H, Pow, Prd, static_cast<int>(B_REG));
						TB.firstEnemy = eb;
						TB.num_enemies++;
						continue;
					}
					...
					// make new node and link the last one with it
					temp = ENEMY::Initialize(S, TY, T, H, Pow, Prd, static_cast<int>(B_REG));
					eb->next = temp;
					TB.num_enemies++;
					break;
				}
				case 'C':
				{
					...
					if (ec == NULL)
					{
						ec = ENEMY::Initialize(S, TY, T, H, Pow, Prd, static_cast<int>(C_REG));
						TC.firstEnemy = ec;
						TC.num_enemies++;
						continue;
					}
					...
					// make new node and link the last one with it
					temp = ENEMY::Initialize(S, TY, T, H, Pow, Prd, static_cast<int>(C_REG));
					ec->next = temp;
					TC.num_enemies++;
					break;
				}
				case 'D':
				{
					...
					if (ed == NULL)
					{
						ed = ENEMY::Initialize(S, TY, T, H, Pow, Prd, static_cast<int>(D_REG));
						TD.firstEnemy = ed;
						TD.num_enemies++;
						continue;
					}
					...
					// make new node and link the last one with it
					temp = ENEMY::Initialize(S, TY, T, H, Pow, Prd, static_cast<int>(D_REG));
					ed->next = temp;
					TD.num_enemies++;
					break;
				}
